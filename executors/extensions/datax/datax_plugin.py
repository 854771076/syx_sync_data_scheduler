...
class DataXPluginManager:
    def __init__(self,tasks:List[Task],settings:settings={}):
    ...
    def execute_action(self):
        ...
        for future in as_completed(futures):
            try:
                ...
                logger.exception(e)
                continue
    def execute_retry(self,log:Log):
        """执行DataX任务"""
...
class DataXPlugin:
    def __init__(self, task: Task,config,settings):
...
class Reader:
    ...
    def mysql2hive(self:DataXPlugin):
        ...
        if self.settings.get('execute_way') == 'all':
            where = f"{self.task.column_config.update_column}<'{self.settings.get('today').strftime('%Y-%m-%d 00:00:00')}'"
        elif self.settings.get('execute_way') == 'update':
            where = f"{self.task.column_config.update_column} between '{self.settings.get('yesterday').strftime('%Y-%m-%d 00:00:00')}' and '{self.settings.get('today').strftime('%Y-%m-%d 00:00:00')}'"
        elif self.settings.get('execute_way') == 'other':
            where = f"{self.task.column_config.update_column} between '{self.settings.get('start_time')}' and '{self.settings.get('end_time')}'"
        else:
            assert False, f"[datax_plugin]:不支持的执行方式[{self.settings.get('execute_way')}]"
        if self.task.column_config.update_column and self.task.column_config.partition_column:
            querySql= [
