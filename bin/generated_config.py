...
def generate_config_sql(task_config):
    ...
    replace INTO `executors_task` (
        ...
        '{task_config["db"]}',  -- source_db
        '{task_config["table"]}',  -- source_table
        '{task_config["hive_db"]}',  -- target_db
        '{task_config["hive_table"]}',  -- target_table
        '[]',  -- columns (模型默认空列表)
        '[]',  -- exclude_columns (模型默认空列表)
        1,  -- is_add_sync_time
        ...
        '{task_config.get('cdc_sync_date') if task_config.get('cdc_sync_date') else 'cdc_sync_date'}',  -- sync_time_column
...
if __name__ == "__main__":
    ...
    with open(sql_file, 'w') as f:
        for table_config in TABLES:
            # 分表逻辑，可根据实际需求调整
            if table_config.get("t_start")==2016:
                split_config_id=4
            elif not table_config.get("db_num") and not table_config.get("t_num"):
                split_config_id=2
                
            elif table_config.get("db_num") and not table_config.get("t_num"):
                split_config_id=5
            elif not table_config.get("db_num") and table_config.get("t_num"):
                if table_config.get("t_num")==255:
                    split_config_id=1
                elif table_config.get("t_num")==4:
                    split_config_id=11
                else:
                    split_config_id=7
            elif table_config.get("db_num") and table_config.get("t_num"):
                split_config_id=3
            else:
                raise Exception("异常分库分表")
            if table_config.get("suffix_list"):
                if table_config.get('table')=='im_customer_info':
                    split_config_id=12
                elif table_config.get('table')=='qt_quality_test_list':
                    split_config_id=13
                elif table_config.get('table')=='om_calldetail_list':
                    split_config_id=14
